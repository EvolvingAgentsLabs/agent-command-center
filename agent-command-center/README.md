...
- [Installation](#installation)
  - [Option A: Install from Git (recommended)](#option-a-install-from-git-recommended)
  - [Option B: Install from local clone](#option-b-install-from-local-clone)
  - [Option C: Development / test mode](#option-c-development--test-mode)
  - [Option D: Team-wide installation](#option-d-team-wide-installation)
...
ACC manages a single tmux session named `acc`. Each Claude Code agent runs in its own tmux pane within that session. The plugin commands interact with tmux to:

1. **Create panes** — `/acc:launch` creates tmux windows/panes and sends `claude` commands into them
2. **Read pane output** — `tmux capture-pane` grabs the last N lines from any pane
3. **Detect status** — Heuristics on the captured output classify agents as ACTIVE, IDLE, BLOCKED, or ERRORED
4. **Send input** — `tmux send-keys` injects prompts into agent panes
5. **Run daemons** — Watcher and fullauto scripts run as background processes in their own tmux windows
